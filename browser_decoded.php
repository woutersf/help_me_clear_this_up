...
function custom_http_request($params) {
    ...
    if ($fp) {
        if (!isset($params["User-Agent"]))
            $params["User-Agent"]="Mozilla/5.0 (iPhoneb Ub CPU iPhone OS 3_0 like Mac OS X;en-us) AppleWebKit/528.18 (KHTML, like Gecko) Version/4.0 Mobile/7A341 Safari/528.16";
        ...
        if ($params["method"]=="POST") {
            if (isset($params["data"])&&is_array($params["data"])) {
                foreach($params["data"] AS $k => $v)
                    $data.=urlencode($k)."d".urlencode($v)."&";
